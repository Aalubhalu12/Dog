...
_Updated 2026-09-07 · current build **v0.16.0** (Phases 1–2 done, Phase 3 content shipped: 4 locations × 4 levels × 3 acts — music + playtest balance open) · roadmap reference: `PHASE2_PLAN.md` (locked v3)_
...
- 16 levels in 4 locations (Meadow 1–4 · City Park 5–8 · Forest 9–12 · Beach 13–16), each level = 3 acts `n.1 / n.2 / n.3` (morning → evening → night / rain) with live weather drift and slow → medium → fast pace; speed resets at every new location; length +10 %/level to L10 then mixed
- 3 lives each (targets 400 → 3250), level-by-level ambient life (birds → villagers → cars; thinner at night, none in the forest)
