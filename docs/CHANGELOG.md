...
- **Installable app (PWA):** `manifest.webmanifest` (fullscreen, portrait, id `/Dog/`), icons 192/512 + maskable +
  apple-touch (generated from the home puppy — `assets/icons/`), and `sw.js`: pre-caches the whole app shell
  (html, css, js, level JSON, every image — 132 files, list generated by `tools/bump.py` so it never drifts),
  cache-first for same-origin, old version caches purged on activate. **Plays fully offline.** A new deploy shows
  "✨ New version ready — tap to update". Settings gets an **Install** row (Android/desktop) or the iOS
  Share → Add-to-Home-Screen hint; an offline pill appears at the bottom when the network drops.
- **Daily bonus** (`src/services/daily.js`): 7-day ladder 20 · 30 · 40 · 60 · 80 · 100 · **200** coins, one claim per
  local day. **Soft streak:** miss a day → back one rung, never to zero. Opens itself once per session on the home
  screen (after FTUE); 🎁 button with a red dot while a claim is open. Coins go through Wallet (`reason: 'daily'`).
- **Profile** (🐶 button): rename (14 chars, feeds the leaderboard), country flag, stars x/48, best, coins, levels
  cleared, streak, bones caught, minutes played; Sign-in / Restore placeholders explain what arrives in Phase 5/slice B.
- **Back navigation** (`src/services/nav.js`): one history sentinel, so Android back / browser back / Escape closes
  the top-most modal, pauses a run, and on the bare home screen asks "press back again to exit" (2 s). Result cards
  (pause/win/over) are sticky — back never skips them.
- Settings → About: Credits, GitHub link, version. `[hidden]` utility, `.btn-mini`, `.btn-x` close.
### Changed
- `tools/bump.py` now also rewrites `sw.js` (VERSION + SHELL). Run it with no argument to just refresh the shell list.
- `tools/sim_play.py` claims the daily popup like a player (26 checks).
